...
/**
 * TODO
 * @brief For given level n, print all nodes on level n (root is on level 0).
 * Nodes should be printed left to right, separated by a space.
 *
 * @param b_tree for which to print level
 * @param n level to be computed
 * @return void
 */

void printGivenLevel(b_node_t* root, int level)
{
    if (root == NULL)
        return;
...
void b_tree_print_level(b_tree_t *b_tree, int n)
{
    if(!b_tree){
        return;
    }
    printGivenLevel(b_tree->root, n);
}
...
int main(void)
{
    ...
    b_tree_print_level(binary_tree, level);
