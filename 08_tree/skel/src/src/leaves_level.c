 ...
 * TODO
 *
 * @brief Helper function for determining whether a tree has level
 * leaves.
 *
 * NOTE: There are multiple methods for solving this problem.
 * Implement however you wish.
 *
 * @param b_tree to check level leaves for
 * @return unsigned 1 if leaves are level, else 0
 */
unsigned int __b_tree_level_leaves(b_node_t *b_node)
{
    if (!b_node)
        return 0;
...
/**
 * @brief Checkes whether all leaves are on same level and prints:
 *
 * "YES\n" if positive
 * "NO\n" if negative or error
 *
 * @param b_tree whose leaves to check if they are on same level
 */
void b_tree_level_leaves(b_tree_t *b_tree)
{
    if (!b_tree)
        return;
    (__b_tree_level_leaves(b_tree->root) == b_tree->data_size) ? printf("YES\n") : printf("NO\n");
}
...
int main(void)
{
    ...
    b_tree_level_leaves(binary_tree);
