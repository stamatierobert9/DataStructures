...
b_node_t* __b_tree_lca(b_node_t* root, int n1, int n2) {
    if(root == NULL){
        return NULL;
    }
    ...
    b_node_t *left = __b_tree_lca(root->left, n1, n2);
    b_node_t *right = __b_tree_lca(root->right, n1, n2);
...
int b_tree_lca(b_tree_t *b_tree, int n1, int n2)
{
    ...
    b_node_t* lca = __b_tree_lca(root, n1, n2);
